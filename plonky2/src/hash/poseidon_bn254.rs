...
pub const BN254_FULL_ROUNDS:usize = 8;
pub const BN254_PARTIAL_ROUNDS:usize = 56;
pub const BN254_SPONGE_WIDTH:usize = 4;
pub const BN254_SPONGE_RATE:usize = 3;
...
impl Poseidon for BN254Field{

    const BN254_FAST_PARTIAL_FIRST_ROUND_CONSTANT: [u64; 12]  = [
    ...
    type PoseidonState = [BN254Field; BN254_SPONGE_WIDTH];

    fn permutation(state: &mut PoseidonState) {
        ark(state, 0);
    ...
    fn full_rounds(state: &mut PoseidonState, first: bool) {
        for i in 0.. BN254_FULL_ROUNDS / 2 - 1 {
            exp5_state(state);
            if first {
                ark(state, (i + 1) * BN254_SPONGE_WIDTH);
            } else {
                ark(
                    state,
                    (BN254_FULL_ROUNDS / 2 + 1) * BN254_SPONGE_WIDTH + BN254_PARTIAL_ROUNDS + i * BN254_SPONGE_WIDTH,
                );
            }
            mix(state, &M_MATRIX);
        }
        ...
        exp5_state(state);
        if first {
            ark(state, (BN254_FULL_ROUNDS / 2) * BN254_SPONGE_WIDTH);
            mix(state, &P_MATRIX);
        } else {
            mix(state, &M_MATRIX);
        }
    ...
    fn partial_rounds(state: &mut PoseidonState) {
        for i in 0..BN254_PARTIAL_ROUNDS {
            state[0] = exp5(state[0]);
            state[0].add_assign(&C_CONSTANTS[(BN254_FULL_ROUNDS / 2 + 1) * BN254_SPONGE_WIDTH + i]);
            let mut mul;
            let mut new_state0 = Fr::ZERO;
            for j in 0..BN254_SPONGE_WIDTH {
                mul = Fr::ZERO;
                mul.add_assign(&S_CONSTANTS[(BN254_SPONGE_WIDTH * 2 - 1) * i + j]);
                mul.mul_assign(&state[j]);
                new_state0.add_assign(&mul);
            }

            for k in 1..BN254_SPONGE_WIDTH {
                mul = Fr::ZERO;
                mul.add_assign(&state[0]);
                mul.mul_assign(&S_CONSTANTS[(BN254_SPONGE_WIDTH * 2 - 1) * i + BN254_SPONGE_WIDTH + k - 1]);
    ...
    fn mix(state: &mut PoseidonState, constant_matrix: &[Vec<Fr>]) {
        let mut result: PoseidonState = [Fr::ZERO; BN254_SPONGE_WIDTH];

        let mut mul;
        for (i, result_element) in result.iter_mut().enumerate().take(BN254_SPONGE_WIDTH) {
            for j in 0..BN254_SPONGE_WIDTH {
                mul = Fr::ZERO;
        ...
        state[..BN254_SPONGE_WIDTH].copy_from_slice(&result[..BN254_SPONGE_WIDTH]);
    }
